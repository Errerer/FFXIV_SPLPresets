...
namespace SplatoonScriptsOfficial.Duties.Endwalker.The_Omega_Protocol
{
    public unsafe class Dynamis_Delta : SplatoonScript
    {
        ...
        public override Metadata? Metadata => new(8, "Errer MGL打法版本(简体中文） 拉线平行 锁链1/2诱导远世界 攻击1/2 诱导近世界 脚本原作者：NightmareXIV");
        ...
        public override void OnSetup()
        {
            ...
            Controller.RegisterElementFromCode("Bait", "{\"Name\":\"\",\"Enabled\":false,\"radius\":0.0,\"color\":3355508735,\"overlayBGColor\":4278190080,\"overlayTextColor\":4294967295,\"overlayFScale\":2.0,\"thicc\":5.0,\"overlayText\":\"引導\",\"tether\":true}");
            Controller.RegisterElementFromCode("Alert", "{\"Enabled\":false,\"Name\":\"\",\"type\":1,\"radius\":0.0,\"overlayBGColor\":4278190080,\"overlayTextColor\":4294967295,\"overlayVOffset\":3.0,\"overlayFScale\":2.0,\"thicc\":0.0,\"overlayText\":\"危險\",\"refActorType\":1}");
            ...
            if (Controller.Scene == 6)
            {
                ...
                if (beetle != null || FakeParty.Get().Any(x => x.HasEffect(Effects.GreenTether)))
                {
                    if (Stage == 0 && (HasEffect(Effects.UpcomingBlueTether) || HasEffect(Effects.UpcomingGreenTether)))
                    {
                        ...
                        {
                            Alert("蓝线去光头!", ImGuiColors.TankBlue);
                            Controller.GetElementByName("Final").Enabled = true;
                        }
                        else
                        {
                            Alert("绿线去蟑螂！", ImGuiColors.HealerGreen);
                    ...
                    else if (Stage == 1)
                    {
                        ...
                        {
                            Alert("换到另一边！", GradientColor.Get(ImGuiColors.ParsedPink, 0xFF000000.ToVector4(), 200));
                        }
                        else
                        {
                            if (myTether == Effects.UpcomingBlueTether)
                            {
                                ...
                                {
                                    Alert("和队友快速重叠！", GradientColor.Get(0xff000000.ToVector4(), ImGuiColors.ParsedPurple, 200));
                                    var pl = GetClosestPlayer();
                                    if (Controller.TryGetElementByName("Stack partner", out var e) && pl != null)
                                    {
                                        e.Enabled = true;
                                ...
                                if (HasEffect(Effects.BlueTether))
                                {
                                    if (IsAnyoneUnsafe)
                                    {
                                        Alert("等待debuff后拉线!", Colors.Red.ToVector4());
                                    }
                                    else
                                    {
                                        Alert("准备去引导", GradientColor.Get(ImGuiColors.DalamudRed, ImGuiColors.DalamudYellow, 200));
                                    }
                                }
                            }
                            else
                    ...
                    else if (Stage == 2)
                    {
                        ...
                        if (arms.Length == 6)
                        {
                            ...
                            if (myTether == Effects.UpcomingBlueTether)
                            {
                                if (isMeClose)
                                {
                                    Alert("中间引导盾连击！", ImGuiColors.DalamudRed);
                                }
                                else
                                {
                                    Alert("诱导手臂激光", ImGuiColors.DalamudOrange);
                            ...
                            else
                            {
                                ...
                                {
                                    Alert("诱导手臂激光", ImGuiColors.DalamudOrange);
                                    myArm = arms.OrderBy(x => Vector3.Distance(x.Position, beetle.Position)).ToArray()[2..4].OrderBy(x => Vector3.Distance(Player.Position, x.Position)).First();
                                }
                                else
                                {
                                    Alert("诱导手臂激光", ImGuiColors.DalamudOrange);
                    ...
                    else if (Stage == 3)
                    {
                        if (HasEffect(Effects.BlueTether))
                        {
                            if (IsAnyoneUnsafe)
                            {
                                Alert("等待连线debuff炸开!", Colors.Red.ToVector4());
                            }
                            else
                            {
                                Alert("准备去引导!", GradientColor.Get(ImGuiColors.DalamudRed, ImGuiColors.DalamudYellow, 200));
                        ...
                        if (myTether == Effects.UpcomingBlueTether)
                        {
                            ...
                            {
                                Alert("中间分摊！", ImGuiColors.HealerGreen);
                            }
                            else
                            {
                                Alert("躲避中间分摊和小电视半场", ImGuiColors.DPSRed);
                                Controller.GetElementByName("Final").Enabled = true;
                            }
                        }
                        else
                        {
                            Alert("小电视屏幕引导", ImGuiColors.HealerGreen);
                        }
                        if (!FakeParty.Get().Any(x => x.HasEffect(Effects.MonitorLeft) || x.HasEffect(Effects.MonitorRight)))
                        {
                            Stage = 4;
                    ...
                    else if (Stage == 4)
                    {
                        ...
                        {
                            Alert("近世界", ImGuiColors.HealerGreen);
                        }
                        else if (HasEffect(Effects.FarWorld))
                        {
                            Alert("远世界", ImGuiColors.ParsedBlue);
                        }
                        else
                        {
                            if (myTether == Effects.UpcomingBlueTether)
                            ...
                            else
                            {
                                if (isMeClose)
                                {
                                    if (HasEffect(Effects.GreenTether))
                                    {
                                        if (IsAnyoneUnsafe)
                                        {
                                            Alert("等待连线debuff拉断连线", Colors.Red.ToVector4());
                                        }
                                        else
                                        {
                                            Alert("靠近拉断绿线", GradientColor.Get(ImGuiColors.DalamudRed, ImGuiColors.DalamudYellow, 200));
                                        }
                                    }
                                    else
                                    {
                                ...
                                {
                                    Alert("去诱导近世界", ImGuiColors.HealerGreen);
                                }
                            }
                        }
                    ...
                    else if (Stage == 5)
                    {
                        if (HasEffect(Effects.GreenTether))
                        {
                            if (IsAnyoneUnsafe)
                            {
                                Alert("等待debuff消失拉断连线", Colors.Red.ToVector4());
                            }
                            else
                            {
                                Alert("靠近拉断绿线", GradientColor.Get(ImGuiColors.DalamudRed, ImGuiColors.DalamudYellow, 200));
