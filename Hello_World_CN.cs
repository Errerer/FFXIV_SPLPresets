...
{
    public class Hello_World_CN : SplatoonScript
    {
        ...
        public override void OnUpdate()
        {
            ...
            if (Svc.Objects.Any(x => x is BattleChara b && b.CastActionId == 31599))
            {
                ...
                if (HasEffect(Effects.RedRot))
                {
                    ...
                    {
                        if(Conf.EnableOverheadHintsGeneric) Reminder("進紅塔 !! 大圈 !!", ImGuiColors.DalamudRed);
                    }
                    else
                    {
                        if (Conf.EnableOverheadHintsGeneric) Reminder("進紅塔 >> 分攤 <<", ImGuiColors.DalamudRed);
                    }
                }
                else if (HasEffect(Effects.BlueRot))
                {
                    ...
                    if (HasEffect(Effects.Defamation))
                    {
                        if (Conf.EnableOverheadHintsGeneric) Reminder("進藍塔 !! 大圈 !!", ImGuiColors.TankBlue);
                    }
                    else
                    {
                        if (Conf.EnableOverheadHintsGeneric) Reminder("進藍塔 >> 分攤 <<", ImGuiColors.TankBlue);
                    }
                }
                else if (HasEffect(Effects.UpcomingCloseTether, 10f))
                {
                    ...
                    if (counter != 4)
                    {
                        ...
                        if (isDefamationRed)
                        {
                            ...
                            if (Conf.EnableOverheadHintsGeneric) Reminder("紅塔遠離線 - 后拉", ImGuiColors.DalamudRed);
                        }
                        else
                        {
                            if (Conf.EnableVisualElementsTowers) TowerBlue(true);
                            if (Conf.EnableOverheadHintsGeneric) Reminder("藍塔遠離線 - 后拉", ImGuiColors.TankBlue);
                        }
                        if(partner != null)
                        {
                    ...
                    else
                    {
                        ...
                        {
                            if (Conf.EnableVisualElementsTowers) TowerBlue(true);
                            if (Conf.EnableOverheadHintsGeneric) Reminder("藍塔中間分攤 - 先拉", ImGuiColors.TankBlue);
                        }
                        else
                        {
                            if (Conf.EnableVisualElementsTowers) TowerRed(true);
                            if (Conf.EnableOverheadHintsGeneric) Reminder("紅塔中間分攤 - 后拉", ImGuiColors.DalamudRed);
                        }
                    }
                }
                else if (HasEffect(Effects.UpcomingFarTether, 10))
                {
                    ...
                    {
                        if (Conf.EnableVisualElementsTowers) TowerBlue(true);
                        if (Conf.EnableOverheadHintsGeneric) Reminder("藍塔中間分攤" + (counter==4? " - 最終分攤" : " - 分攤"), ImGuiColors.TankBlue);
                    }
                    else
                    {
                        if (Conf.EnableVisualElementsTowers) TowerRed(true);
                        if (Conf.EnableOverheadHintsGeneric) Reminder("紅塔中間分攤" + (counter == 4 ? " - 最終分攤" : " - 分攤"), ImGuiColors.DalamudRed);
                    }
                }
            }
            else
            {
                ...
                if (RotPicker)
                {
                    if(Conf.EnableRotPickerReminding) Reminder("!!! 接毒 !!!", 0xFF000000.ToVector4());
                ...
                else
                {
                    ...
                    {
                        if(Conf.EnableOverheadHintsTether) Reminder("<<< 遠離拉斷 >>>", ImGuiColors.HealerGreen);
                    }
                    if (HasEffect(Effects.CloseTether) && !Svc.Objects.Any(x => x is PlayerCharacter pc && HasEffect(Effects.FarTether)))
                    {
                        if (Conf.EnableOverheadHintsTether) Reminder(">>> 接近拉斷 <<<", ImGuiColors.ParsedBlue);
                    }
                }
            }
        }
